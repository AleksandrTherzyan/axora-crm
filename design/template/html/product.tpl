		...
		<div class="product__meta">
			<div class="meta">
				<div class="meta__item">
					{include file="_rating.tpl" is_product_page=true}
				</div>
				<div class="meta__item">
					<a href="#reviews" class="comment-count js-target-link"><i class="fal fa-comment"></i> <span class="comment-count__val">{$comments|count}</span></a>
				</div>
				<div class="meta__item">
                 {if $brand}
		...
		<div class="product__content">
			...
			<form action="/cart" class="js-submit-to-cart" method="post">
					...
					<div class="product__variants">
						<div class="variants">
							...
							<select class="form-control product__select js-change-variant" name="variant">
								{foreach $product->variants as $v}
									<option
											...
											{foreach $currencies as $c}{if $c->enabled}data-price-currency-{$c->id|escape}="{$v->price|convert:$c->id}"{/if}{/foreach} value="{$v->id}" {if $product->variant->id == $v->id}selected{/if}>{$v->name|escape}
									</option>
								{/foreach}
							</select>
							...
							<div class="variants__thumbs product__select-thumbs">
								{foreach $product->variants as $v}
									{if $v->attachment}
										<span class="variants__thumbs-item {if $product->variant->id == $v->id}is-selected{/if}" data-value="{$v->id}" style="background-image: url({$v->attachment|resize:800:600});"></span>
									{/if}
								{/foreach}
							</div>
						</div>
                ...
                <div class="product__btn-field js-cart_button-render-area">
                    ...
                    {else}
                        <button type="submit" class="basket-btn btn btn-warning {if $is_cart}is-active{/if}" data-alt-text="В корзине">
					...
					<div class="add-review__content" style="{if $error}display:block{/if}">
						<form class="add-review__form"  method="post">
							...
							<div class="form-group">
								...
								<input type="text" class="form-control" id="comment_name" name="name" required value="{$comment_name}">
							</div>

							<div class="form-group">
								<label for="text">Отзыв</label>
								<textarea class="form-control" id="reviewText" name="text" rows="8" required>{$comment_text}</textarea>
							</div>
									...
									<div class="review">
										<div class="review__main">
											<div class="review__header">
												...
												*}
												<div class="review__date">{$comment->date|date}, {$comment->date|time}</div>
												{if !$comment->approved}
...
{get_related_products2 var=related_products product_id=$product->id limit=10 }
{if $related_products}
	<section class="catalog-slider-section">
